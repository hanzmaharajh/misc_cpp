...
TYPED_TEST_SUITE(Cache_size10Test, all_test_types, );
...
TYPED_TEST_SUITE(MultibucketCache_size10Test, multibucket_test_types, );
...
TYPED_TEST(MultibucketCache_size10Test, InsertNoEvictDifferentBucket) {
  ...
  EXPECT_EQ(std::size(this->m_buckets[0]), 1);
  EXPECT_EQ(std::size(this->m_buckets[2]), 1);
}
...
TYPED_TEST(MultibucketCache_size10Test, InsertNoEvictSameBucket) {
  ...
  EXPECT_EQ(std::size(this->m_buckets[0]), 2);
}
...
TYPED_TEST(MultibucketCache_size10Test, InsertEvictDifferentBucket) {
  ...
  EXPECT_EQ(std::size(this->m_buckets[2]), 0);
  EXPECT_EQ(std::size(this->m_buckets[4]), 1);
}
...
TYPED_TEST(MultibucketCache_size10Test, InsertEvictSameBucket) {
  ...
  EXPECT_EQ(std::size(this->m_buckets[3]), 1);
...
TYPED_TEST_SUITE(Cache_MoveTest, all_test_types, );
...
TYPED_TEST(Cache_MoveTest, MoveKeyWorks) {
  {
    ...
    EXPECT_EQ(std::data(this->m_keys_to_locators.find(str_a_copy)->first),
              data_ptr);
  }

  {
    ...
    EXPECT_NE(std::data(this->m_keys_to_locators.find(str_a_copy)->first),
              data_ptr);
  }
}
...
TEST_F(SizeAndPopularityAwareCache_size10Test, Evict_Unpopular) {
  ...
  EXPECT_EQ(std::size(this->m_buckets[2]), 0);
  EXPECT_EQ(std::size(this->m_buckets[1]), 1);
  for (size_t i = 0; i < 4; ++i) {
    this->fetch(1);
  }
  EXPECT_EQ(std::size(this->m_buckets[1]), 0);
  EXPECT_EQ(std::size(this->m_buckets[0]), 1);
  // It's popular!
