...
template <typename Mutex = std::mutex, typename CondVar = std::condition_variable>
class semaphore {
public:
    explicit semaphore(size_t size) 
        : m_capacity(size) {}
...
public:
    [[nodiscard]]
    explicit semaphore_lock(Semaphore& sem, size_t num) 
    : m_sem(sem), 
      m_num(num) 
    {
        m_sem.acquire(m_num);
